...
while True:
    ...
    for encoding in encodings:
     ...
     #      print("Found match")
        ...
        if True in matches:
            ...
            print("[INFO][{}] Identified: {} during hour {}, {}".format(time, name, now.hour, daypart))
    ...
    #            cv2.imwrite(filename_box, frame)
            ...
            if greeting_date_dict[name] != daypart:
                print("[INFO] No greeting yet, so using: {}".format(message + name))
                message = "Good " + daypart + ", " + name
                speak(message)
                filename = name + "_" + time + ".jpg"
                greeting_date_dict[name] = daypart
            else:
                print("[INFO][{}] Already greeted {} for the {}".format(time, name, daypart))
